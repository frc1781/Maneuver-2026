...
// Prevent this function from intercepting the homepage or other routes
...
export const handler: Handler = async (event) => {
  if (event.path !== "/.netlify/functions/api-proxy") {
...
const rawProvider = (event.queryStringParameters?.provider || '').toLowerCase();

const provider = rawProvider === 'tba' || rawProvider === 'nexus' || rawProvider === 'statbotics'
  ? rawProvider
  : undefined;
    ...
    const providerNeedsApiKey = provider !== undefined;
  ...
  ...(provider === 'tba' && apiKey ? { 'X-TBA-Auth-Key': apiKey } : {}),
  ...(provider === 'nexus' && apiKey ? { 'Nexus-Api-Key': apiKey } : {}),
  ...(provider === 'statbotics' && apiKey ? { Authorization: `Bearer ${apiKey}` } : {}),
};
